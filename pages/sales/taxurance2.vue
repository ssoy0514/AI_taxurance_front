...
const TAXURANCE_API_BASE =
  process.env.TAXURANCE_API_BASE !== undefined
    ? process.env.TAXURANCE_API_BASE
    : 'http://localhost:8003'
const taxuranceClient = axios.create({ baseURL: TAXURANCE_API_BASE })
    ...
    async startStreaming() {
      ...
      try {
        if (this.submitType === 'first') {
          try {
            const { data: openingData } = await taxuranceClient.post('/api/generate/opening', {
        ...
        const { data: finalData } = await taxuranceClient.post('/api/generate/final', {
          interest: this.resultFilters.interest,
          sex: this.resultFilters.sex,
          age_label: this.resultFilters.age,
          considerations: this.resultFilters.considerations,
